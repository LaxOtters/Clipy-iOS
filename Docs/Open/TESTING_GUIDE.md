...
UI test는 unit test로 옮기기 어려운 검증을 모아두는 곳이 아닙니다.
실제 앱 조립과 사용자 입력을 거쳐야만 확인할 수 있는 핵심 플로우에만 남깁니다.
시뮬레이터 실행 시간과 flaky test 대응 비용까지 포함해, 더 가벼운 테스트로 같은 문제를 잡을 수 없는지 먼저 봅니다.

화면 검증 방법은 맡는 역할이 다릅니다.

| 방법 | 맡는 역할 |
| --- | --- |
| Unit / state test | 화면에 드러나는 제품 규칙과 상태 전환을 빠르게 검증합니다. |
| UIKit component test | 한 컴포넌트의 공개 입력과 출력만 좁게 검증합니다. |
| Manual check / PR screenshot | 현재 구현이 디자인과 맞는지 사람이 확인합니다. |
| Snapshot test | 자주 깨질 위험이 있고 비교 기준이 안정적인 대표 화면을 검증합니다. |
| Headless simulator 검증 | 구현이나 리뷰 시점에 실제 렌더링과 입력을 확인합니다. 상시 test suite에 남긴다는 뜻은 아닙니다. |
| UI test | 실제 앱에서 여러 화면과 입력을 지나야만 확인되는 핵심 사용자 플로우를 검증합니다. |

### 상시 UI test로 남기는 경우

아래 조건을 모두 만족할 때 UI test를 우선 검토합니다.

- 사용자가 실제로 수행하는 플로우를 안정적으로 재현할 수 있습니다.
- 여러 화면의 조립, 네비게이션, 실제 입력 전달 중 하나가 검증의 핵심입니다.
- unit, state, component, snapshot test로는 같은 실패를 관찰할 수 없습니다.
- 회귀 위험이 시뮬레이터 실행 시간과 flaky test 관리 비용보다 큽니다.
- 테스트 전용 화면이나 제품 코드의 별도 디버그 라우트 없이 실제 앱 진입점에서 시작할 수 있습니다.

UI test가 필요하더라도 가능한 조합을 모두 만들지 않습니다.
제품 위험을 대표하는 1~2개 플로우부터 남기고, 각 테스트가 막는 회귀를 이름에서 읽을 수 있게 합니다.

다음 항목은 기본적으로 상시 UI test 대상이 아닙니다.

- Figma와 색상, 간격, 폰트가 맞는지 확인하는 작업
- 버튼 하나의 콜백 연결이나 단일 화면의 보임/숨김
- 큐, 중복 제거, 타이머, 생명주기처럼 UI 밖에서 결정적으로 검증할 수 있는 규칙
- private view hierarchy나 constraint 값
- 실제 기능 화면에 연결되지 않은 공용 컴포넌트
- 한 번 확인하면 충분한 애니메이션과 화면 전환의 시각 품질

### 구현 중 headless 확인

상시 UI test가 없어도 실제 UIKit 렌더링과 터치 전달을 확인해야 할 수 있습니다.
이때는 시뮬레이터를 headless로 실행해 필요한 시나리오만 확인하고, 결과를 구현 또는 리뷰 증빙으로 남깁니다.
이 검증은 현재 변경이 동작한다는 근거이지, 같은 시나리오를 계속 유지할 UI test로 채택했다는 뜻은 아닙니다.

실제 기능 화면에 연결된 뒤 여러 화면을 지나는 회귀가 생기고, 더 가벼운 테스트로 관찰할 수 없을 때 상시 UI test 여부를 다시 봅니다.

### 테스트를 위한 제품 코드 변경과 flaky test

UI test를 위해 제품 코드에 별도 화면 이동 경로, 테스트 전용 상태, 넓어진 접근 범위를 추가하지 않습니다.
`accessibilityIdentifier`는 실제 접근성 또는 지속해서 사용할 UI 자동화 경계일 때만 둡니다.

flaky test는 재시도로 가리지 않습니다.
제품 코드의 비결정성인지 실행 환경 문제인지 먼저 구분합니다.
테스트를 제외해야 한다면 해당 테스트가 지키던 제품 규칙을 다른 테스트나 명시적인 검증 절차로 남깁니다.
